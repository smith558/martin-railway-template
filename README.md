...
- `docker-entrypoint.sh`: validates env, generates the runtime Martin config, and starts Martin
...
The runtime Martin config is generated by `docker-entrypoint.sh`; the checked-in `martin.yaml` is reference-only.
...
If you need to change publish rules, CORS, route prefixes, caching, or add other sources, update `docker-entrypoint.sh` and redeploy.
