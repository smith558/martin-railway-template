...
case "${DATABASE_URL}" in
  *"channel_binding=require"*)
    echo "warning: DATABASE_URL uses channel_binding=require. In earlier testing, this caused Martin to fail PostgreSQL startup. If Martin does not boot, try removing channel_binding=require from DATABASE_URL." >&2
    ;;
esac
